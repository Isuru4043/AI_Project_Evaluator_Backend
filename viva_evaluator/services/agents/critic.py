...
class CriticInput:
    ...
    module_chunks: List[Dict] = None           # Module boundary checks
...
def critique_question(inp: CriticInput) -> Dict:
    ...
    bound = _clamp01(response.get('boundary_check_score', 1.0))
    halluc = bool(response.get('hallucination_flag', False))

    # Critic passes if all quality bars are met
    quality_ok = spec >= 0.5 and bloom >= 0.5 and conv >= 0.5 and bound >= 0.5 and not halluc
